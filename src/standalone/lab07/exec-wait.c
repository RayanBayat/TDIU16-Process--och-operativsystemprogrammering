...
struct running_thread {
  ...
  struct semaphore* mysemaphone;                                            // Rayan      Heap
...
void thread_main(struct running_thread *data) {
  ...
  sema_up(data->mysemaphone);                                                // Rayan      Heap
...
struct running_thread *exec(int param) {
  ...
  // Skapa en ny tråd som kör "thread_main" och ge den tillgång till "data".
  thread_new(&thread_main, data);
  data ->mysemaphone = malloc(sizeof(struct semaphore));

  sema_init(data->mysemaphone,0);                                          // Rayan        Heap
  return data;
}
...
int wait(struct running_thread *data) {
  ...
  sema_down(data->mysemaphone);                                              // Rayan      Heap      
  int result = data->result;
  free(data->mysemaphone);
  free(data);                                                               // Rayan      Heap
  return result;
}
